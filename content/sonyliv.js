...
function findCards() {
  ...
  console.debug(`[StreamRatings/SonyLIV] findCards found ${found.size} cards on ${location.pathname}`);
  return found;
}
...
function processNewCards() {
  ...
  for (const card of cards) {
    ...
    if (!parsed) {
      console.debug("[StreamRatings/SonyLIV] Could not extract title from card:", card.outerHTML.slice(0, 200));
      continue;
    }
  ...
  if (newCount > 0) {
    console.debug(`[StreamRatings/SonyLIV] Processing ${newCount} new cards`);
  }
}
...
async function fetchAndInjectRating(card, title, year) {
  console.debug(`[StreamRatings/SonyLIV] Fetching rating for: "${title}" (year: ${year ?? "unknown"})`);
  ...
  if (!data) {
    console.debug(`[StreamRatings/SonyLIV] No data returned for "${title}" — API key missing?`);
    return;
  }

  const rating = data.imdbRating;
  if (!rating || rating === "N/A") {
    console.debug(`[StreamRatings/SonyLIV] N/A rating for "${title}", skipping badge`);
    return;
  }

  console.debug(`[StreamRatings/SonyLIV] Injecting badge for "${title}": ${rating}`);
